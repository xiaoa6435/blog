...
分位数回归的标准误比较复杂。这里我们反过来，先考虑最复杂也最通用的场景出发，然后逐步简化，得到更简单情况下的标准误差。 $\{(y_{gi}, x_{gi})\}, g = 0, 1, 2, ..., (n_g - 1)$，表示各个整群，每个整群分别有$n_g$个个体。这是分位数模型可以表示为：
...
这里$B = f_{\mu_{\tau}}(0|X)X^{'}X$, $f_{\mu_{\tau}}$是残差在0处的概率密度, $A = Xe^{'}Xe$。 Xe的计算方式如下：对每个整群，分别计算$\sum_{n=0}^{n_g - 1} X * \phi(\tau) = \sum_{n=0}^{n_g - 1} X * (\tau - \mathbb{1}(y - x\beta(\tau)))$，因此Xe的维度是[G, p], G是整群的个数。 
...
其中, $H = f_{\mu_{\tau}}(0|X)X^{'}X$, $J = X^{'}X$, 其中$f_{\mu_{\tau}}$式残差在0处的概率密度。如果残差是独立同分布的，即i.i.d, 这个公式可以简化为：
